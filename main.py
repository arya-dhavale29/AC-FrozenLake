...
def main():
    ...
    lake = [['&', '.', '.', '.'],
            ...
            ['.', '.', '.', '#'],
            ['#', '.', '.', '$']]
    env = FrozenLake(lake, slip=0.1, maxsteps=16, seed=seed)
    gamma = 0.9
    print('# Model-based algorithms')
    print('')
    print('## Policy iteration')
    policy, value = policy_values.policy_iteration(env, gamma, theta=0.001, max_iterations=128)
    env.render(policy, value)
    print('')
    print('## Value iteration')
    policy, value = policy_values.value_iteration(env, gamma, theta=0.001, max_iterations=128)
    env.render(policy, value)
    print('')
